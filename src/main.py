...
def import_data(item: DocumentItem):
    try:
        addDocument(item)
        return {"status": "success", "message": f"Document {item.id} imported successfully."}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/search")
def search_data(query: str, n_results: int = 1):
...
def search_data_updated(query: str, n_results: int = 1):
...
def ollama_query_with_context(query: str, n_results: int = 3):
    if validate_input(query):
        return {
            "answer": "Prompt is not accurate"
        }
    answer = generateAnswerFromQueryWithContext(query, n_results, "ollama")
    answer = validate_output(answer)
    return {"answer": answer}

@app.post("/gemini-query-with-context")
def gemini_query_with_context(query: str, n_results: int = 3):
    if not isPromptValid(query):
        return {
            "answer": "Prompt is not accurate"
        }
    answer = generateAnswerFromQueryWithContext(query, n_results,"gemini")
    return {"answer": answer}
